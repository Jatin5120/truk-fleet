...
class TrukDocumentController {
  ...
  Future<Map<String, dynamic>> getPerticularUploadStatus({String id}) async {
    ...
    return {
      TrukDocumentType.insurance: [
        m.containsKey(TrukDocumentType.insurance),
        m[TrukDocumentType.insurance]
      ],
      TrukDocumentType.rc: [
        m.containsKey(TrukDocumentType.rc),
        m[TrukDocumentType.rc]
      ],
      TrukDocumentType.permit: [
        m.containsKey(TrukDocumentType.permit),
        m[TrukDocumentType.permit]
      ],
      TrukDocumentType.fitness: [
        m.containsKey(TrukDocumentType.fitness),
        m[TrukDocumentType.fitness]
      ],
    };
  }
  ...
  Future<String> uploadDocument(String type, File file) async {
    ...
    TaskSnapshot uploadTask =
        await storage.ref().child('documents/$fileName.$ext').putFile(file);
